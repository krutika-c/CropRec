...
# Fertilizer recommendations per nutrient deficiency
# Structure: { nutrient: { fertilizer_name, dose_kg_per_acre, timing, method } }
FERTILIZER_DATA = {
    'N_low': [
...
# Fertilizer translations: {lang: {key: translated_string}}
FERT_TRANSLATIONS = {
    'English': {
        ...
        'dose_label': 'Dose', 'timing_label': 'Timing', 'method_label': 'Method', 'unit': 'mg/kg',
    },
    'Hindi': {
        ...
        'status_good': 'good', 'status_low': 'low', 'status_excess': 'excess', 'status_high': 'high',
        'dose_label': 'खुराक', 'timing_label': 'समय', 'method_label': 'तरीका', 'unit': 'मिग्रा/किग्रा',
    },
    'Telugu': {
...
@app.route('/geocode')
def geocode():
    """Proxy for Nominatim geocoding — bypasses browser CORS restrictions."""
...
def predict():
    try:
        ...
        fertilizer = build_fertilizer_suggestions(prediction, data, language)
        crop_name = get_crop_name(prediction, language)

        print(f"[Predict] crop={prediction}, confidence={confidence}, lang={language}")
        return jsonify({'crop': prediction, 'crop_name': crop_name, 'confidence': confidence,
...
def yield_query():
    try:
        ...
        if suitability_score >= 75:
            ...
            outlook_label = t(language, 'outlook_good')
        elif suitability_score >= 50:
            outlook, color = "Moderate", "yellow"
            outlook_label = t(language, 'outlook_moderate')
        elif suitability_score >= 25:
            outlook, color = "Poor", "orange"
            outlook_label = t(language, 'outlook_poor')
        else:
            outlook, color = "Very Poor", "red"
            outlook_label = t(language, 'outlook_very_poor')

        analysis = build_yield_analysis(crop, data, suitability_score, rank, best_crop, language)
        fertilizer = build_fertilizer_suggestions(crop, data, language)
        ...
        return jsonify({
            'crop': crop, 'crop_name': crop_name, 'suitability_score': suitability_score,
            'rank': rank, 'total_crops': len(KNOWN_CROPS),
            'outlook': outlook, 'outlook_label': outlook_label, 'outlook_color': color,
            'best_alternative': best_crop, 'best_alternative_name': best_alt_name,
            'analysis': analysis,
